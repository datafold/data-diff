...
C.execute(f'CREATE TABLE {schema}.tmp_rating ("userid" DECIMAL(18,0), "movieid" DECIMAL(18,0), "rating" DOUBLE PRECISION, "timestamp" DECIMAL(18,0))')

print('Import csv from s3 into tmp_rating table...')
C.import_from_file(table=(schema,'tmp_rating'), src='dev/ratings.csv', import_params={'skip': 1, 'column_separator': ',', 'row_separator': 'CRLF'})

print("Create rating table from tmp_rating...")
C.execute(f'SELECT ROW_NUMBER() OVER (order by "userid", "movieid", "timestamp") AS "id", "userid", "movieid", "rating", "timestamp"  INTO TABLE {schema}."rating" FROM {schema}.tmp_rating')
...
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_del1" FROM {schema}."rating"')
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_update1" FROM {schema}."rating"')
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_update001p" FROM {schema}."rating"')
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_update1p" FROM {schema}."rating"')
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_del1p" FROM {schema}."rating"')
C.execute(f'SELECT "id", "userid", "movieid", "rating", "timestamp" INTO TABLE {schema}."rating_update50p" FROM {schema}."rating"')
...
assert C.execute(f'SELECT COUNT(*) FROM {schema}."rating"').fetchval() == C.execute(f'SELECT COUNT(*) FROM {schema}."rating_del1"').fetchval() + 1
